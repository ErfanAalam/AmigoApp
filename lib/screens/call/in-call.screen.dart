...
class _InCallScreenState extends ConsumerState<InCallScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Handle call ended states - navigate back
    if (activeCall == null ||
      ...
      if (activeCall != null) {
        switch (activeCall.status) {
          ...
          case CallStatus.missed:
            statusMessage = 'Call missed';
            break;
          default:
            statusMessage = 'Closing call...';
        }
      }
    ...
    // Determine if this is an outgoing call that hasn't been answered yet
    final bool isOutgoingRinging =
        activeCall.callType == CallType.outgoing &&
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            Expanded(
              flex: 1,
              child: isOutgoingRinging
                  ? // Outgoing call not yet answered - show cancel button only
                    Center(
                      child: _buildCancelCallButton(
                        onPressed: () => _cancelCall(context),
                      ),
                    )
                  : // Connected call - show full controls
                    Row(
  ...
  Widget _buildCancelCallButton({required VoidCallback onPressed}) {
    return GestureDetector(
      onTap: onPressed,
      child: Container(
        width: 80,
  ...
  void _cancelCall(BuildContext context) async {
    try {
      // For outgoing calls that haven't been answered, use decline (which will cancel)
      await ref
          .read(callServiceProvider.notifier)
          .declineCall(reason: 'caller_cancelled');
      // Don't manually navigate here - let the Consumer handle it
    } catch (e) {
