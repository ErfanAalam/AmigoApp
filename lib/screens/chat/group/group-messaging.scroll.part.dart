...
extension _GroupScroll on _InnerGroupChatPageState {
  ...
  void _onScroll() {
    ...
    if (_isInJumpMode && !_isScrollingToJumpTarget) {
      ...
      if (distanceFromTop <= 1000) _loadMoreMessages();
    }
  }
  ...
  void _highlightMessage(String messageId) {
    ...
    _highlightTimer = Timer(const Duration(milliseconds: 2000), () {
      if (mounted) _safeSetState(() => _highlightedMessageId = null);
    });
  }
  ...
  Future<void> _jumpToMessage(String messageId) async {
    ...
    try {
      ...
      if (result.data == null) {
        ...
        Snack.warning('Could not load message');
        return;
      }
      ...
      if (mounted) _safeSetState(() => _isLoadingTargetMessage = false);
  ...
  Future<void> _loadJumpOlderMessages() async {
    ...
    try {
      ...
      if (!mounted || !_isInJumpMode) return;
      if (result.data != null) {
        ...
        if (history.messages.isNotEmpty) {
          _safeSetState(() {
            ...
            if (_jumpMessages.length > _InnerGroupChatPageState._jumpWindowSize) {
              final excess = _jumpMessages.length - _InnerGroupChatPageState._jumpWindowSize;
      ...
      debugPrint('[Group] _loadJumpOlderMessages: $e');
    } finally {
      if (mounted) _safeSetState(() => _isLoadingJumpOlder = false);
    }
  }
  ...
  Future<void> _loadJumpNewerMessages() async {
    ...
    try {
      ...
      debugPrint('[Group] _loadJumpNewerMessages: $e');
    } finally {
      if (mounted) _safeSetState(() => _isLoadingJumpNewer = false);
    }
  }
