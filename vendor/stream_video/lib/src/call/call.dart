...
class Call {
  ...
  Future<void> _handleCoordinatorCallAccepted(
    ...
    if (event.acceptedByUserId == currentUserId &&
        ...
        !status.acceptedByMe) {
      // [AMIGO-PATCH] Original behaviour was:
      //   await reject(reason: CallRejectReason.userRespondedElsewhere());
      //
      // That path is meant for "user accepted on another device" — but the
      // same condition fires for a single-device accept too: the WS broadcast
      // of our own accept arrives BEFORE `lifecycleCallAccepted()` flips
      // `acceptedByMe` to true. Result: the SDK self-rejects the call the
      // user just answered, the caller sees "Call declined", and the callee
      // ends up in a ghost call. Suppressing this path is safe for our app —
      // we don't support simultaneous accepts across multiple devices.
      _logger.i(
        () =>
            '[onCoordinatorEvent] [AMIGO-PATCH] self-accept event received — '
            'skipping userRespondedElsewhere reject; letting local '
            'lifecycleCallAccepted finish',
      );
      return;
    }
  ...
  Future<void> _handleCoordinatorCallRejected(
    ...
    if (event.rejectedByUserId == currentUserId &&
        status is CallStatusIncoming &&
        !status.acceptedByMe) {
      // [AMIGO-PATCH] Same race as the accept-side: the WS broadcast of our
      // own reject() can fire before local state catches up. The SDK
      // originally re-issued reject(userRespondedElsewhere); pointless second
      // hit on the coordinator. No-op is safe — terminal status is already
      // being driven by the original reject.
      _logger.i(
        () =>
            '[onCoordinatorEvent] [AMIGO-PATCH] self-reject event received — '
            'skipping userRespondedElsewhere reject',
      );
      return;
    }
  ...
  Future<Result<None>> _doJoin({
    ...
    if (result.isFailure) {
      ...
      // [AMIGO-PATCH] Trace which path triggered the failure so we can see
      // it in logs. The original code blindly calls reject(timeout()) on
      // ANY _awaitIfNeeded failure — including cancellation from _clear
      // (which already disconnected the call) and lifecycle completion
      // from a leave() that's already in flight. That second reject ends
      // the call from the caller's user — which the coordinator broadcasts
      // to the OTHER party as "Call declined", killing the call before it
      // has a chance to ring.
      // ignore: avoid_print
      print('[AMIGO-PATCH] _doJoin: _awaitIfNeeded FAILED — '
          'currentStatus=${state.value.status.runtimeType}  '
          'lifecycleCompleted=${_callLifecycleCompleter.isCompleted}  '
          'leaveTriggered=$_leaveCallTriggered  '
          'failureResult=$result');

      // Suppress the spurious self-reject when the call was already left or
      // the status is already Disconnected. In those cases, the coordinator
      // already knows or will know via the natural leave() path; firing a
      // separate reject() just races and breaks the other side.
      final alreadyDisconnected = state.value.status is CallStatusDisconnected;
      if (_callLifecycleCompleter.isCompleted ||
          _leaveCallTriggered ||
          alreadyDisconnected) {
        // ignore: avoid_print
        print('[AMIGO-PATCH] _doJoin: skipping reject(timeout) — '
            'call is being torn down via another path');
        return result;
      }
  ...
  Future<Result<None>> _awaitIfNeeded() async {
    ...
    // ignore: avoid_print
    print('[AMIGO-PATCH] _awaitIfNeeded ENTRY  cid=${state.callCid.value}  '
        'status=${status.runtimeType}  '
        'autoCancelTimeout=${settings.ring.autoCancelTimeout}  '
        'autoRejectTimeout=${settings.ring.autoRejectTimeout}');
  ...
  Future<Result<None>> leave({DisconnectReason? reason}) async {
    try {
      // ignore: avoid_print
      print('[AMIGO-PATCH] Call.leave() ENTRY  cid=${this.state.value.callCid.value}  '
          'reason=$reason  '
          'currentStatus=${this.state.value.status.runtimeType}  '
          'leaveTriggered=$_leaveCallTriggered  '
          'lifecycleCompleted=${_callLifecycleCompleter.isCompleted}\n'
          '${StackTrace.current}');
      if (_leaveCallTriggered) {
