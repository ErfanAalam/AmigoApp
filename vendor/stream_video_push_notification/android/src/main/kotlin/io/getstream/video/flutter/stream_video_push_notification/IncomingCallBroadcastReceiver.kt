        ...
        when (action) {
            ...
            "${context.packageName}.${IncomingCallConstants.ACTION_CALL_DECLINE}" -> {
                try {
                    ...
                    // [AMIGO-PATCH] In killed state, the Flutter event above
                    // goes into the void — there's no isolate to call
                    // call.reject() against Stream's coordinator, so the
                    // caller stays in ringing limbo until Stream's natural
                    // ring timeout fires call.missed (~20-30 s). We bridge
                    // the gap with a fire-and-forget HTTP POST to our own
                    // backend (`/call/stream/decline-cold`), which holds
                    // the API secret and re-signs a one-shot JWT for the
                    // user to call Stream's reject API server-side.
                    AmigoColdDeclineBridge.notifyDecline(context, data)
                } catch (error: Exception) {
                    Log.e(TAG, null, error)
                }
            }
